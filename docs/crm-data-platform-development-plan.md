...
```json
{
  ...
  "serviceScope": ["성능점검 대행", "유지보수관리 위탁"],
...
5. Supabase `inquiries` 저장
6. `inquiry_events`에 `saved_to_db` 기록
7. Web3Forms 또는 대체 메일 발송
8. 메일 결과를 `inquiry_events`에 기록
9. 접수 결과 반환
...
접수 API는 실패를 숨기지 않고 이벤트로 남긴다.

- DB 저장 실패: `db_save_failed`
- 메일 발송 실패: `email_failed`
- 검증 실패: `validation_failed`
- 중복 의심: `duplicate_detected`
